...
We explored our response variable (following year life expectancy) to analzye the distribution as well as check for missingness.
```{r}
# Load package(s) ----
library(tidymodels)
library(tidyverse)
...
We used two recipes-- the first being an all-encompassing kitchen sink model, and the second with the addition of step_corr to remove highly correlated variables above a threshold of 0.5. 
...
This solidifies our choice to go with recipe 1. Because some of the RMSEs are very similar, we used run times to make a further decision.
